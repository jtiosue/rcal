...
from rcal.alternate import calibrate_parameters, CalibrationParameters
...
def test_errors():
    ...
    cp = CalibrationParameters(
        {('c', 'r1'): 1, ('e', 'r1'): 1, ('gamma', 'p1'): 0, ('beta', 'p1'): 2}
    )
...
def test_simple_cases():
    ...
    assert cp == cp1
    cp2 = cp.copy()
    cp2.parameters[('c', 'r1')] = cp.parameters[('c', 'r1')] - 1
    assert cp != cp2
...
def test_ignore_outliers():
    ...
    data[('r2', 'p1', 1)] = 5
    cp = calibrate_parameters(train_data)
    cp.rescale_parameters(data, ignore_outliers=2)
    calibrated_data = cp.calibrate_data(data)
